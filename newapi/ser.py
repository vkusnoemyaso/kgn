...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:am20,ir03!@127.0.0.1/bd_1'
...
def generate_qr():
    ...
    qr_data_string = f"""
    Номер телефона получателя: {recipient_phone_number}
    Имя получателя: {recipient_name}
    Количество груза: {cargo_col}
    Описание груза: {cargo_description}
    Количество чеков: {check_number}
    Дата заполнения: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}
    Выбор кузова: {truck_type}
    Цена: {price}
    Комментарий: {comment}
    Имя водителя: {user.username}
    Номер телефона водителя: {user.phone_number}
    Номер фуры: {user.cargo_number}
    """
